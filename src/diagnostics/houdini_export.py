...
def export_simulator_state(
    ...
    *,
    ...
    # particle's fragment_id so they aren't visually orphaned.  The
    # GaussianSplitter records `_last_append_parent_idx`: each new slot
    # at index >=N_surface points to its parent particle index in
    # [0, N_surface).  Without this remap the floaters render with
    # fid=-1 and look like "floating gaussians" in the viewer.
    # Densified Gaussians (slots past surface count) inherit their parent
    # particle's fragment_id so they aren't visually orphaned.  Parent
    # indices are surfaced by either:
    #   - splitter._last_append_parent_idx  (crack-split debris)
    #   - gaussians._interior_parent_idx     (interior fill Gaussians)
